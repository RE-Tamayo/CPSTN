<?php
    ...
    if(isset($_GET['id'])) {
        $id = $_GET['id'];
        $review = new AdminApproveContr();
        $data = $review->getReg($id);
    } else{
        header('Location: /admin?login=Session expired.');
        exit();
    }
    ...
    <main>
        ...
        <div class="card shadow bg-light cs-12 rs-7">
            <span class="card-header d-flex justify-content-between">
                ...
                <a href="/admin/approve" class="btn btn-primary text-light font-weight-bold">
                    <i class="fa-solid fa-arrow-left" id="refresh-icon"></i>
                    <span class="ml-2" id="refresh-text">Back</span>
                </a>
            </span>
            </div>
        </div>
